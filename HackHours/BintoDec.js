...
const bind2dec = (num) => {
  ...
  for (let i = 0; i < num.length; i++) {
    result += 2 ** (num.length - 1 - i) * num[i];
  }

  return result;
};

console.log(bind2dec('1011011'));



function binToDec(binary, num = 0) {
  if (binary.length === 0) return num;  
  num += 2 ** (binary.length-1) * binary[0]
  return binToDec(binary.slice(1), num)
}


console.log(binToDec('1011011'));
